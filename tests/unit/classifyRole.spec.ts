...
import { classifyRole } from '../../src/matcher/classifyRole';

test.describe('classifyRole', () => {
  test('classifies QA jobs by testing keywords', () => {
    expect(classifyRole('QA Junior Engineer', 'Write tests with Playwright and Cypress')).toBe('qa');
  ...
  test('classifies backend jobs', () => {
    expect(classifyRole('Backend Developer', 'Node.js REST APIs with SQL databases')).toBe('backend');
  ...
  test('classifies full stack when both frontend and backend appear', () => {
    expect(classifyRole('Full Stack Developer', 'React frontend and Node.js backend')).toBe('fullstack');
    expect(classifyRole('Software Engineer', 'You will build the react frontend and the node.js backend with sql')).toBe('fullstack');
  });

  test('classifies mobile, data, devops and support jobs', () => {
    expect(classifyRole('Mobile Developer', 'React Native apps for Android and iOS')).toBe('mobile');
    ...
    expect(classifyRole('Help Desk Analyst', 'Service desk tickets and troubleshooting')).toBe('support');
  });
  ...
  test('returns unknown when no signal is present', () => {
    expect(classifyRole('Office Assistant', 'Organize paperwork and answer phones')).toBe('unknown');
  });
});
